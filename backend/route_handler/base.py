...
class RouteHandler(DAORetrieveAssertingOwnershipMixin):
    ...
    def get_stripe_client_for_request(
        ...
        effect_mode = self.app.get_effect_mode(request)
        if effect_mode == "sandbox":
            return self.app.stripe_client_sandbox
        elif effect_mode == "live":
            return self.app.stripe_client_live
        raise RuntimeError("Unexpected effect mode")
